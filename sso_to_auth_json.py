...
用法:
  # 鉴权（并发）
  python3 sso_to_auth_json.py --mode auth --sso sso_list.txt --out-dir ./xai_credentials --workers 10 --retries 3 --retry-interval 5
...
def process_auth_one(
    ...
    wait_s = max(0.0, float(retry_interval or 0))
    last_err = ""
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            if not token:
                ...
                path = out_dir / credential_file_name(
                ...
                if attempt > 1:
                    log(f"{prefix} ✅ {label} → {path.name} ({size}B) [第{attempt}次成功]")
                else:
        ...
        if attempt < attempts:
            log(f"{prefix} ⚠️ {last_err}，{wait_s:g}s 后重试 ({attempt}/{attempts})")
            if wait_s > 0:
                time.sleep(wait_s)
        else:
            log(f"{prefix} ❌ {last_err}（已重试 {attempts} 次）")
    return False


def process_noauth_one(
    idx: int,
    total: int,
    line: str,
    out_dir: Path,
...
def run_pool(
    ...
    retry_interval = max(0.0, float(retry_interval or 0))

    if workers == 1 or total == 1:
        for i, item in enumerate(items, 1):
            if worker_fn(i, total, item, out_dir, email, retries, retry_interval):
                ok += 1
            else:
                fail += 1
        return ok, fail
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = [
            ex.submit(worker_fn, i, total, item, out_dir, email, retries, retry_interval)
            for i, item in enumerate(items, 1)
        ]
...
def main() -> int:
    ...
    ap.add_argument("--retries", type=int, default=3, help="失败重试次数（默认 3，含首次）")
    ap.add_argument(
        "--retry-interval",
        type=float,
        default=5,
    ...
    retry_interval = max(0.0, float(args.retry_interval or 0))
    log(f"🚀 {title}")
    log(
        f"📦 条目: {len(items)} | workers={workers} | "
        f"retries={retries} | retry_interval={retry_interval:g}s"
    )
    log(f"📁 输出: {out_dir}")
